...
const OrderDetailCard = ({ orderId, onClose }) => {
  ...
  useEffect(() => {
    if (orderId) {
      const currOrder = ordersList.find((order) => order.id === orderId);
      if (currOrder) {
        setCurrentOrder(currOrder);
        setTrackingData(currOrder.tracking || []);
      }
    }
  }, [orderId, ordersList]);

  if (!currentOrder) {
    return null; // Return null if order not found or still loading
  }
  ...
  async function handleCancelOrder(){
    ...
    const trackData = [...trackingData, {timestamp: Timestamp.now(), status:"cancelled", desc: "Your order cancelled by exclusive", confirmedBy : "Admin", completed:true}]
    const updatedData = {...currentOrder, status:"cancelled", tracking: trackData}
    const {success} = await updateOrdersList(orderId, updatedData)
  }
  ...
  return (
    <div className="bg-slate-50 dark:bg-gray-800 h-full rounded-lg text-gray-800 dark:text-gray-300 border dark:border-gray-600 border-gray-400 overflow-y-auto scrollbar">
      ...
      <div className="px-2">
        ...
        <div className="border-b dark:border-gray-500 border-gray-300 pt-2 pb-4 mt-2 mx-2">
          <div className="flex justify-between items-center">
            ...
            <button onClick={() => setEditTrack(true)}>
          ...
          {editTrack && (
            <Modal onClose={() => setEditTrack(false)}>
              <EditTrack
                id={id}
                initialData={trackingData}
                setTrackingData={setTrackingData}
                onClose={() => setEditTrack(false)}
              />
            </Modal>
          )}
        </div>
